...
class CentralDatabase:
    ...
    def _ensure_history_columns(self, conn, cursor):
        ...
        # Camera type tracking - entrance, exit, internal
        add_col("entry_camera_type", "TEXT")
        add_col("exit_camera_type", "TEXT")
        add_col("last_location_camera_type", "TEXT")
        # Customer type: 'subscription' or 'guest'
        add_col("customer_type", "TEXT DEFAULT 'guest'")
            ...
            try:
                cursor.execute(
                    ...
                    INSERT INTO history (
                        ...
                        entry_confidence, entry_source, entry_camera_type, status, sync_status, customer_type
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'IN', ?, ?)
                    """,
                    (
                        event_id,
    ...
    def update_vehicle_exit(self, plate_id, exit_time, camera_id, camera_name, confidence, source, duration, camera_type=None):
        """Update vehicle exit in history. Returns True if updated."""
        with self.lock:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            ...
            cursor.execute(
                ...
                SET exit_time = ?, exit_camera_id = ?, exit_camera_name = ?,
                    exit_confidence = ?, exit_source = ?, exit_camera_type = ?, duration = ?,
                ...
                """,
                (exit_time, camera_id, camera_name, confidence, source, camera_type, duration, plate_id),
            )
    ...
    def find_vehicle_in_parking(self, plate_id):
        """Find vehicle currently in parking (status=IN, exit_time IS NULL)"""
        with self.lock:
            conn = sqlite3.connect(self.db_file)
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
    ...
    def get_history(self, limit=100, offset=0, today_only=False, status=None, search=None, in_parking_only=False, entries_only=False, from_date=None, to_date=None, start_time=None, end_time=None):
        ...
        with self.lock:
            ...
            # Precise time range filter (start_time and end_time)
            # Logic: Show record if it has an Entry OR an Exit within the range
            # This aligns with Shift Stats (Count In + Count Out)
            # Use DATETIME() to handle different timestamp formats (ISO vs YYYY-MM-DD HH:MM:SS)
            if start_time and end_time:
                query += """ AND (
                    (DATETIME(entry_time) >= DATETIME(?) AND DATETIME(entry_time) <= DATETIME(?))
                    OR
                    (exit_time IS NOT NULL AND DATETIME(exit_time) >= DATETIME(?) AND DATETIME(exit_time) <= DATETIME(?))
                )"""
                params.extend([start_time, end_time, start_time, end_time])
            elif start_time:
                 query += " AND (DATETIME(entry_time) >= DATETIME(?) OR (exit_time IS NOT NULL AND DATETIME(exit_time) >= DATETIME(?)))"
    ...
    def update_history_entry(self, history_id, new_plate_id, new_plate_view, changed_by="system"):
        ...
        with self.lock:
            ...
            try:
                ...
                # Luu lich su thay doi với local time
                from datetime import datetime
                ...
                cursor.execute("""
                    INSERT INTO history_changes (
                        ...
                        new_plate_id, new_plate_view, old_data, new_data, changed_at, changed_by
                    ) VALUES (?, 'UPDATE', ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    history_id,
    ...
    def delete_history_entry(self, history_id, changed_by="system"):
        ...
        with self.lock:
            ...
            try:
                ...
                # Luu lich su thay doi truoc khi xoa với local time
                from datetime import datetime
                changed_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                ...
                cursor.execute("""
                    INSERT INTO history_changes (
                        history_id, change_type, old_plate_id, old_plate_view,
                        old_data, changed_at, changed_by
                    ) VALUES (?, 'DELETE', ?, ?, ?, ?, ?)
                """, (
                    history_id,
                    old_data.get('plate_id'),
    ...
    def get_history_changes(self, limit=100, offset=0, history_id=None, from_date=None, to_date=None, search=None):
        ...
        with self.lock:
            ...
            # Date range filter for changes
            # Date range filter for changes
            if from_date:
                query += " AND DATETIME(changed_at) >= DATETIME(?)"
                params.append(from_date)
            if to_date:
                query += " AND DATETIME(changed_at) <= DATETIME(?)"
                params.append(to_date)
            ...
            if search:
                ...
                query += """ AND (
                    old_plate_id LIKE ? OR 
                    new_plate_id LIKE ? OR 
                    old_plate_view LIKE ? OR 
                    new_plate_view LIKE ?
                )"""
    ...
    def update_vehicle_location(self, plate_id, location, location_time, camera_type=None):
        """
        Update location for vehicle currently in parking lot

        Args:
            plate_id: Vehicle plate ID
            location: Camera name/location
            location_time: Timestamp
            camera_type: Camera type - entrance, exit, internal (optional)

        Returns:
            True if updated, False if vehicle not in parking
        """
        with self.lock:
            conn = sqlite3.connect(self.db_file)
            conn.row_factory = sqlite3.Row
            ...
            if camera_type:
                ...
                """, (location, location_time, camera_type, plate_id))
            else:
                cursor.execute("""
                    UPDATE history
                    SET last_location = ?,
