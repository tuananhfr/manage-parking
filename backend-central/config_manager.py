...
class ConfigManager:
    ...
    def update_config(self, new_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "bank_account" in new_config:
                bank_account = new_config["bank_account"]
                content = self._update_value(content, "BANK_ACCOUNT_NUMBER", bank_account.get("account_number", ""), is_string=True)
                content = self._update_value(content, "BANK_ACCOUNT_NAME", bank_account.get("account_name", ""), is_string=True)
                content = self._update_value(content, "BANK_NAME", bank_account.get("bank_name", ""), is_string=True)
                content = self._update_value(content, "BANK_CODE", bank_account.get("bank_code", ""), is_string=True)
                content = self._update_value(content, "DESCRIPTION_PREFIX", bank_account.get("description_prefix", "Chuyển tiền"), is_string=True)
    ...
    def _update_value(self, content: str, key: str, value: Any, is_string: bool = False) -> str:
        ...
        if is_string:
            # Try to match os.getenv() pattern first
            pattern_getenv = rf'^{key}\s*=\s*os\.getenv\([^)]+\)'
            if re.search(pattern_getenv, content, flags=re.MULTILINE):
                # Update os.getenv pattern: KEY = os.getenv("KEY", "default")
                replacement = f'{key} = os.getenv("{key}", "{value}")'
                content = re.sub(pattern_getenv, replacement, content, flags=re.MULTILINE)
            else:
                # Fallback to simple string pattern: KEY = "value"
                pattern = rf'^{key}\s*=\s*"[^"]*"'
