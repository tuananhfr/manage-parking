...
# Import new routes for Camera RTSP, Timelapse, Parking Backends, NVR Servers, Auth
from routes import camera_routes, timelapse_routes, parking_backend_routes, nvr_routes, parking_lock_proxy_routes, bank_account_routes, auth_routes, report_routes
...
app.include_router(timelapse_routes.router)  # Already has prefix="/api/timelapse"
app.include_router(parking_backend_routes.router)
app.include_router(nvr_routes.router)
app.include_router(parking_lock_proxy_routes.router)
app.include_router(bank_account_routes.router)
app.include_router(auth_routes.router)
app.include_router(report_routes.router)

# Timelapse videos giờ được serve từ NVRs, không cần mount static files nữa

# --- Added for Frontend Compatibility (AI Cameras) ---
...
async def websocket_cameras(websocket: WebSocket):
    ...
    try:
        ...
        camera_websocket_clients.remove(websocket)

@app.get("/api/cameras")
async def get_cameras():
    """Fallback endpoint for AI cameras hooks"""
...
async def _proxy_webrtc_to_nvr(nvr_id: str, camera_id: str, payload: Dict[str, Any]):
    """
    Proxy WebRTC offer/answer to NVR's go2rtc instance
    
    Args:
        nvr_id: NVR server ID
        camera_id: Camera stream ID in go2rtc
        payload: WebRTC offer/answer payload
    
    Returns:
        WebRTC answer from NVR's go2rtc
    """
    # Load NVR servers
    nvr_file = os.path.join(os.path.dirname(__file__), "nvr.servers.json")
    try:
        with open(nvr_file, 'r', encoding='utf-8') as f:
            servers = json.load(f)
    except:
        raise HTTPException(status_code=500, detail="Failed to load NVR servers")
    
    # Find NVR
    nvr = next((s for s in servers if s['id'] == nvr_id), None)
    if not nvr:
        raise HTTPException(status_code=404, detail=f"NVR server {nvr_id} not found")

    # Use go2rtc_url from config, fallback to auto-generate from host
    go2rtc_url = nvr.get('go2rtc_url') or f"http://{nvr['host']}:1984"

    # Build go2rtc WebRTC endpoint
    endpoint = f"{go2rtc_url}/api/webrtc?src={camera_id}"
    timeout = 10.0
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="invalid_response_from_nvr_go2rtc"
        ) from err
...
# WebRTC Proxy Endpoint for NVR streams
@app.post("/api/webrtc/nvr/{nvr_id}/{camera_id}")
async def webrtc_proxy_nvr(nvr_id: str, camera_id: str, request: Request):
    """
    Proxy WebRTC offer to NVR's go2rtc instance
    
    Usage from frontend:
    POST /api/webrtc/nvr/nvr-001/camera-a
    Body: { "type": "offer", "sdp": "..." }
    
    Returns: WebRTC answer from NVR's go2rtc
    """
    try:
        payload = await request.json()
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Invalid JSON payload: {str(e)}")
    return await _proxy_webrtc_to_nvr(nvr_id, camera_id, payload)

@app.get("/api/webrtc/nvr/{nvr_id}/{camera_id}")
async def webrtc_proxy_nvr_get(nvr_id: str, camera_id: str):
    """
    Health check endpoint for WebRTC NVR proxy
    
    Returns: Info about the endpoint (GET is not supported, use POST instead)
    """
    raise HTTPException(
        status_code=405,
        detail="Method Not Allowed. Use POST method with WebRTC offer payload: { 'type': 'offer', 'sdp': '...' }"
    )
...
async def receive_heartbeat(request: Request):
    """
    Nhận heartbeat từ Edge camera HOẶC Unified App (NVR)

    Body (Camera): { "camera_id": 1, ... }
    Body (NVR): { "device_id": "parking-edge-001", "host": "192.168.0.x", "port": 5000, ... }
    """
    global camera_registry
    ...
    try:
        ...
        # Check for NVR Heartbeat (Auto-Sync) - DISABLED per user request
        # device_id = data.get('device_id')
        # ... logic removed ...

        # If it's purely an NVR heartbeat (no camera_id), return success (Ignored)
        if not data.get('camera_id'):
            return JSONResponse({"success": True, "type": "nvr_heartbeat_ignored"})
        ...
        # Kiểm tra xem camera có trong config không
        # Nếu không có trong config → không nhận heartbeat và xóa khỏi database (camera đã bị xóa)
        edge_config = _get_edge_camera_config(camera_id)
        if not edge_config:
            print(f"[Heartbeat] Ignoring heartbeat from camera {camera_id} - not in config (camera may have been deleted)")
            # Xóa camera khỏi database nếu vẫn còn (cleanup)
            global database
            if database:
                try:
                    database.delete_camera(camera_id)
                    print(f"[Heartbeat] Deleted camera {camera_id} from database (not in config)")
                except Exception as delete_err:
                    print(f"[Heartbeat] Error deleting camera {camera_id}: {delete_err}")
            return JSONResponse({
                "success": False,
                "error": f"Camera {camera_id} not found in config"
            }, status_code=404)

        # Update heartbeat chỉ khi camera có trong config
...
def _sync_nvr_from_heartbeat(device_id, host, port, nvr_id=None, name=None, description=None):
    """Helper to update NVR config from heartbeat"""
    import json
    import os
    ...
    try:
        ...
        # 1. Try to find by nvr_id (Primary ID)
        if nvr_id:
        ...
        # 2. If not found, find by device_id (Legacy/Fallback)
        ...
        if target_server:
            ...
            # Check ID mismatch (if matched by device_id but nvr_id is different)
            if nvr_id and s.get('id') != nvr_id:
                print(f"[NVR Auto-Sync] Warning: NVR found by device_id={device_id} has id={s.get('id')}, but heartbeat reports nvr_id={nvr_id}. Keeping existing ID.")
                # We do NOT rename ID to avoid breaking links. User should fix manually or delete old one.

            # Update fields
            if s.get('host') != host or s.get('port') != port:
                s['host'] = host
                s['port'] = port
                s['go2rtc_url'] = f"http://{host}:1984"
                changed = True
            ...
            if description and s.get('description') != description:
                s['description'] = description
                changed = True
                
            servers[target_index] = s
            if changed:
                print(f"[NVR Auto-Sync] Updated NVR {s['id']} ({s['name']})")
                
        else:
            # Create new
            new_id = nvr_id or f"nvr_{device_id}"
            new_name = name or f"NVR {device_id}"
            
            print(f"[NVR Auto-Sync] Auto-registering new NVR: {new_id} ({new_name}) at {host}:{port}")
            ...
            new_server = {
                ...
                "port": port,
                "device_id": device_id,
                "go2rtc_url": f"http://{host}:1984",
                "go2rtc_rtsp_port": 8554,
                "go2rtc_webrtc_port": 8555,
                "cameras": [],
                "description": description or "Auto-registered from heartbeat",
                "enabled": True
            }
        ...
        if changed:
            os.makedirs(os.path.dirname(nvr_file), exist_ok=True)
            with open(nvr_file, 'w', encoding='utf-8') as f:
                json.dump(servers, f, indent=2, ensure_ascii=False)
                
    except Exception as e:
        print(f"[NVR Auto-Sync] Error: {e}")
...
async def receive_ocr_from_unified_app(request: Request):
    ...
    Unified_app gửi:
    {
        ...
        "camera_name": "khu a",
        "plate_text": "29A12345",
        "camera_type": "entrance",  # entrance, exit, internal
        "timestamp": "2024-12-24T13:58:30.123456"
    }

    Logic:
    - Tìm xe với plate_text đang status='IN' trong bảng history
    - Nếu tìm thấy → cập nhật last_location = camera_name, last_location_time = timestamp, camera_type
    ...
    try:
        ...
        camera_type = data.get('camera_type', 'internal')  # Default: internal
        ...
        print(f"[OCR] Received from {device_id}/{camera_name} ({camera_type}): {plate_text} (normalized: {plate_id})")

        # Logic tùy theo camera_type
        if camera_type == "entrance":
            # Camera VÀO → Tạo entry nếu chưa vào, bỏ qua nếu đã vào
            existing = database.find_vehicle_in_parking(plate_id)
            if existing:
                # Xe đã vào rồi → BỎ QUA (không báo lỗi, đây là hành vi bình thường)
                print(f"[OCR] Vehicle {plate_id} already inside, skipping (entry time: {existing.get('entry_time')})")
                return JSONResponse({
                    "success": True,
                    "action": "SKIP",
                    "message": f"Vehicle {plate_text} already inside",
                    "already_inside": True,
                    "entry_time": existing.get("entry_time")
                })

            # Xe chưa vào → Tạo entry record
            entry_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            # Check if subscription
            from parking_state import _check_subscription
            customer_type = _check_subscription(plate_id)
            
            history_id = database.add_vehicle_entry(
                plate_id=plate_id,
                plate_view=plate_text,
                entry_time=entry_time,
                camera_id=camera_name,  # Use camera_name for consistency
                camera_name=camera_name,
                confidence=0.9,
                source="unified_app",
                camera_type=camera_type,
            ...
            if history_id:
                print(f"[OCR] ✓ ENTRY created for {plate_id} at {camera_name}")
                try:
                    ...
                    print(f"Failed to broadcast: {broadcast_err}")
            ...
            # Camera RA → Xử lý exit nếu đã vào, bỏ qua nếu chưa vào
            existing = database.find_vehicle_in_parking(plate_id)
            if not existing:
                # Xe chưa vào (hoặc đã ra rồi) → BỎ QUA (không báo lỗi, đây là hành vi bình thường)
                print(f"[OCR] Vehicle {plate_id} not inside, skipping exit")
                return JSONResponse({
                    "success": True,
                    "action": "SKIP",
                    "message": f"Vehicle {plate_text} not inside (hasn't entered or already exited)",
                    "has_entry": False
                })

            # Xe đã vào → Xử lý exit
            exit_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            entry_time = existing.get('entry_time')
            duration, fee = parking_state._calculate_fee(entry_time, exit_time)

            # Update exit
            updated = database.update_vehicle_exit(
                plate_id=plate_id,
                exit_time=exit_time,
                camera_id=camera_name,  # Use camera_name for consistency
                camera_name=camera_name,
            ...
            if updated:
                print(f"[OCR] ✓ EXIT recorded for {plate_id} at {camera_name}, fee={fee}")
                try:
                    await broadcast_history_update({
                        "event_type": "EXIT",
                        "plate_id": plate_id,
            ...
            # Camera TRONG BÃI (internal) → Chỉ cập nhật vị trí
            ...
            if vehicle:
                ...
                # Xe CHƯA VÀO hoặc ĐÃ RA → BỎ QUA (không báo lỗi, đây là hành vi bình thường)
                print(f"[OCR] Vehicle {plate_id} not inside, skipping location update")
                return JSONResponse({
                    "success": True,
                    "action": "SKIP",
                    "message": f"Vehicle {plate_text} not inside (hasn't entered or already exited)"
...
async def delete_camera(camera_id: int):
    ...
    try:
        deleted = database.delete_camera(camera_id)
        if deleted:
            # Broadcast camera update để frontend refresh
            await broadcast_camera_update()
            return JSONResponse({
                "success": True,
...
async def get_parking_history(
    ...
    """Get vehicle history with optional search by plate number and date range"""
    global database
...
async def delete_history_entry(history_id: int, changed_by: str = "system"):
    ...
    try:
        ...
        success = database.delete_history_entry(history_id, changed_by)
...
async def get_stats():
    ...
    # [MODIFIED] Inject aggregated revenue from Parking Lock backends
    try:
        from routes.parking_lock_proxy_routes import get_all_payment_stats
        # Get stats for today
        today_str = datetime.now().strftime("%Y-%m-%d")
        payment_stats = await get_all_payment_stats(start_date=today_str, end_date=today_str)
        
        # Override revenue_today with aggregated total_revenue
        if payment_stats:
            stats["revenue_today"] = payment_stats.get("total_revenue", 0)
    except Exception as e:
        print(f"Failed to fetch aggregated payment stats: {e}")
        # Keep existing revenue (which is 0) if failed
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        # Neu API loi (403, etc.), fallback ve file JSON
                        logging.warning(f"Staff API returned {response.status_code}, falling back to local JSON")
            except Exception as api_error:
                logging.warning(f"Staff API error: {api_error}, falling back to local JSON")
        
        # Doc tu file JSON (hoac fallback tu API loi)
...
async def sync_staff_from_url(request: Request):
    """
    Save staff API URL to config and fetch staff data from it.
    Uses saved Drupal session from login instead of requiring credentials.
    """
    import config as config_module
    import os
    ...
    try:
        ...
        # Load saved Drupal session
        drupal_cookies = load_drupal_session()
        if not drupal_cookies:
            return JSONResponse({
                "success": False,
                "error": "Không có session Drupal. Vui lòng đăng nhập lại vào hệ thống trước."
            }, status_code=400)
        
        logging.info(f"[Staff Sync] Attempting to sync from: {api_url}")
        logging.info(f"[Staff Sync] Using saved session with {len(drupal_cookies)} cookies")
        
        # Save to config first
        config_manager.update_config({
            "staff": {"api_url": api_url}
        })
        ...
        # Fetch from API with saved Drupal session
        async with httpx.AsyncClient(timeout=30.0, cookies=drupal_cookies) as client:
            response = await client.get(api_url)
            
            logging.info(f"[Staff Sync] Staff API response status: {response.status_code}")
            
            if response.status_code == 403:
                return JSONResponse({
                    "success": False,
                    "error": "Session đã hết hạn. Vui lòng đăng xuất và đăng nhập lại."
                }, status_code=400)
            ...
            # Transform to staff.json format (simple mapping, no ID generation)
            staff_json_file = config_module.STAFF_JSON_FILE
            json_path = os.path.join(os.path.dirname(__file__), staff_json_file)
            ...
            if os.path.exists(json_path):
                try:
                    with open(json_path, 'r', encoding='utf-8') as f:
                        old_staff_list = json.load(f)
                        for s in old_staff_list:
                            name = s.get("name")
                            if name:
                                existing_status_map[name] = s.get("status", "inactive")
                except Exception as e:
                    logging.warning(f"Failed to load existing staff for status mapping: {e}")
            ...
            for shift in shift_list:
                ...
                # Shift formatting (Time range)
                start_time_str = shift.get("field_start_now", "")
                end_time_str = shift.get("field_end_time", "")
                
                # Format to friendly string "HH:MM DD/MM/YYYY" for JSON storage
                start_time_display = ""
                end_time_display = ""
                
                try:
                    # Parse ISO format: "2026-01-15T08:00:51"
                    if start_time_str:
                        start_dt = datetime.strptime(start_time_str, "%Y-%m-%dT%H:%M:%S")
                        start_time_display = start_dt.strftime("%H:%M %d/%m/%Y")
                        
                    if end_time_str:
                        end_dt = datetime.strptime(end_time_str, "%Y-%m-%dT%H:%M:%S")
                        end_time_display = end_dt.strftime("%H:%M %d/%m/%Y")
                            
                except Exception as e:
                    logging.warning(f"Error parsing shift time: {e}")
                
                # Preserve existing status
                status = existing_status_map.get(username, "inactive")

                new_staff_list.append({
                    "name": username,
                    "position": position,
                    "shift_name": shift.get("title", ""), # Keep original title for reference/coloring logic
            ...
            logging.info(f"Synced {len(new_staff_list)} staff entries from API via /api/staff/sync")
            
            return JSONResponse({
                "success": True,
                "message": f"Đã đồng bộ {len(new_staff_list)} người trực từ API",
                "staff": new_staff_list
            })
...
async def sync_subscriptions_from_drupal():
    """Sync subscriptions from Drupal API"""
    from routes.auth_routes import load_drupal_session
    import config as config_module
    import os
    ...
    try:
        ...
        if not drupal_cookies:
            return JSONResponse({
                "success": False,
                "error": "Không có session Drupal. Vui lòng đăng nhập lại vào hệ thống."
            }, status_code=400)
        
        # Drupal API URL for vehicles
        api_url = "https://paristechno.vn/api/v1/vehicles?_format=json"
        ...
        async with httpx.AsyncClient(timeout=30.0, cookies=drupal_cookies) as client:
            ...
            vehicle_list = api_data if isinstance(api_data, list) else []
            ...
            for idx, vehicle in enumerate(vehicle_list, start=1):
                plate = vehicle.get("field_bien_so", "")
                owner = vehicle.get("field_car_owner", "")
                phone = vehicle.get("field_phone_number", "")
            ...
            logging.info(f"Synced {len(new_subscriptions)} subscriptions from Drupal")
            
            return JSONResponse({
                "success": True,
                "message": f"Đã đồng bộ {len(new_subscriptions)} thuê bao từ Drupal",
...
# Include Authentication router (BEFORE protected routes)
app.include_router(auth_routes.router)
